...
class HeightmapPhysicsComponent extends mix(PhysicsComponent).with()
{
    ...
    Initialise()
    {
		...
		try
		{
			const originalArray = geometry.getAttribute("position").array;
			...
			for(let y = this.m_BodySettings.Divisions; y >= 0; y--)
			{
				...
				{
					originalArray[index] = this.m_BodySettings.HeightMap[x][y];
					index+=3;
				}
			}
		...
		if(geometry.getAttribute("position").array[0] === -(this.m_BodySettings.Size/2))
		{
